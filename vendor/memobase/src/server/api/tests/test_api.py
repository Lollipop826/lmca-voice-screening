...
def test_blob_api_deduplicates_source_turn_id(client, db_env):
    response = client.post(f"{PREFIX}/users", json={})
    user_id = response.json()["data"]["id"]
    payload = {
        "blob_type": "doc",
        "blob_data": {"content": "only once"},
        "fields": {"source_turn_id": "source-turn-1"},
    }

    first = client.post(f"{PREFIX}/blobs/insert/{user_id}", json=payload).json()
    second = client.post(f"{PREFIX}/blobs/insert/{user_id}", json=payload).json()

    assert first["errno"] == second["errno"] == 0
    assert first["data"]["id"] == second["data"]["id"]
    assert second["data"]["duplicate"] is True
    response = client.get(
        f"{PREFIX}/users/blobs/{user_id}/{BlobType.doc}?page=0&page_size=10"
    )
    assert len(response.json()["data"]["ids"]) == 1
