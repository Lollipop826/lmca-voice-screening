...
def pack_blob_from_db(blob: GeneralBlob, blob_type: BlobType) -> Blob:
    ...
    fields = getattr(blob, "additional_fields", None)
    match blob_type:
        case BlobType.chat:
            return ChatBlob(**blob_data, fields=fields, created_at=blob.created_at)
        case BlobType.doc:
            return DocBlob(**blob_data, fields=fields, created_at=blob.created_at)
        case BlobType.summary:
            return SummaryBlob(**blob_data, fields=fields, created_at=blob.created_at)
        case _:
