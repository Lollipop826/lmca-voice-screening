...
def _source_meta(user_event_gist: UserEventGist) -> dict:
    event = getattr(user_event_gist, "event", None)
    event_data = getattr(event, "event_data", None) if event is not None else None
    event_data = event_data if isinstance(event_data, dict) else {}
    return {
        "event_id": user_event_gist.event_id,
        "project_id": user_event_gist.project_id,
        "session_id": event_data.get("session_id"),
        "turn_id": event_data.get("turn_id"),
        "source_turn_id": event_data.get("source_turn_id"),
    }
...
async def search_user_event_gists(
    ...
    time_range_in_days: int | None = None,
) -> Promise[UserEventGistsData]:
    ...
    if time_range_in_days is not None:
        filters.append(
    ...
    with Session() as session:
        ...
        TRACE_LOG.info(
            project_id,
            user_id,
            f"Event Query: {query}; embedding_ms={embedding_ms:.1f}; pgvector_search_ms={search_ms:.1f}",
        )
