...
def _joined_blob_field(blobs: list[Blob], key: str) -> str | None:
    values = []
    for blob in blobs:
        value = (blob.fields or {}).get(key)
...
async def process_blobs(
    ...
    p = await handle_session_event(
        ...
        project_profiles,
        {
            "session_id": _joined_blob_field(blobs, "session_id"),
            "turn_id": _joined_blob_field(blobs, "turn_id"),
            "source_turn_id": _joined_blob_field(blobs, "source_turn_id"),
        },
    )
    if not p.ok():
        return p
...
async def handle_session_event(
    ...
    source_fields = {key: value for key, value in (source_fields or {}).items() if value}
