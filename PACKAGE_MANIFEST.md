...
“林秀兰”档案的 `extra_profile.fictional_demo=true`，并注明不对应真实个人。包内数据库是从原库按 `patient_id` 重新筛选、脱敏并 `VACUUM` 后生成的测试副本：账号、密码哈希、API key、认证 secret、患者分配和其他患者数据均不包含。详见 `TEST_DATA_LIN_XIULAN.md`。
...
2. 按 `HANDOVER.md` 安装 Python 和前端依赖。
3. 先运行测试，再启动服务。
