...
class EmotionTestsetPreparer:
    ...
    def __init__(self, sqlite_db: str, output_dir: str, *, allow_mock: bool = False):
    ...
    def extract_from_sqlite(self, num_samples: int = 100):
        ...
        if not Path(self.sqlite_db).exists():
            ...
            raise FileNotFoundError(
                f"数据库不存在: {self.sqlite_db}；为防止把模拟数据误当评测集，"
                "默认不会自动生成 mock。仅调试时可加 --allow-mock"
            )
        ...
        try:
            cursor.execute(query, (int(num_samples),))
        ...
        if not rows:
            if self.allow_mock:
                ...
                return
            raise RuntimeError(
                "数据库中未找到带音频的对话；默认不会用 mock 替代真实评测数据"
            )
        ...
        for row in rows:
            ...
            if audio_path and Path(audio_path).exists():
                ...
                audio_path_rel = new_audio_path.relative_to(self.output_dir).as_posix()
            else:
            ...
            self.samples.append({
                ...
                # Manifest-relative POSIX path is portable across machines.
                'audio_path': audio_path_rel,
                'text': text,
                # The model's own prediction must never be used as ground
                # truth for evaluating that model. A human annotation is
                # required before this sample becomes valid.
                'true_emotion': None,
    ...
    def _create_mock_testset(self, num_samples: int):
        ...
        for i in range(num_samples):
            ...
            self.samples.append({
                ...
                'audio_path': audio_path.relative_to(self.output_dir).as_posix(),
                'text': template['text'],
    ...
    def add_manual_annotations(self, annotation_file: str):
        ...
        invalid = [
            ...
            if annotation.get('true_emotion') not in self.EMOTION_LABELS
        ]
        if invalid:
            raise ValueError(
                f"人工标注中有 {len(invalid)} 条非法情绪标签"
            )
    ...
    def save_manifest(self):
        ...
        for s in self.samples:
            emo = s.get('true_emotion') or 'unreviewed'
            age = s['age_group']
    ...
    def validate(self) -> bool:
        ...
        for sample in self.samples:
            ...
            if not audio_path.is_absolute():
                audio_path = self.output_dir / audio_path
            ...
            if sample.get('true_emotion') not in self.EMOTION_LABELS:
                print(f"   ❌ 无效情绪标签: {sample['true_emotion']}")
                valid = False
            if sample.get('source') in {'mock', 'sqlite_unreviewed'}:
                print(
                    f"   ❌ 样本尚无人工真值: {sample.get('sample_id')} "
        ...
        if missing_audio > 0:
            ...
            if any(sample.get('source') == 'mock' for sample in self.samples):
                print(f"   这是模拟测试集，需要手动准备音频文件")
                valid = False

        observed = {
            sample.get('true_emotion')
            for sample in self.samples
            if sample.get('true_emotion') in self.EMOTION_LABELS
        }
        missing_labels = sorted(set(self.EMOTION_LABELS) - observed)
        if missing_labels:
            print(f"   ❌ 缺少情绪类别: {missing_labels}")
            valid = False
...
def main():
    ...
    parser.add_argument(
        '--allow-mock',
        action='store_true',
        help='仅用于准备流程调试；允许生成不可用于评测的 mock 清单',
    )
    ...
    preparer = EmotionTestsetPreparer(
        args.sqlite_db,
        args.output_dir,
